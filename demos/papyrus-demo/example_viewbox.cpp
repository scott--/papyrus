...
Example_Viewbox::Example_Viewbox()
{
  ...
  for ( int mos=0; mos <= 1; mos++ )
  {
    for ( int n = 0; n <= 9; n++ )
    {
      ...
      m_Viewport[mos][n][0].canvas()->set_scroll_anchor( Papyrus::SCROLL_ANCHOR_TOP_LEFT );

      d = Papyrus::example_group ( true, false );
      d->set_viewbox ( Papyrus::Viewbox::create ( 0.0, 0.0, 50.0, 25.0, static_cast<Papyrus::Viewbox::ALIGN> ( n ), static_cast<Papyrus::Viewbox::MEET_OR_SLICE> ( mos ) ) );
      m_Viewport[mos][n][0].canvas()->add ( d );
      ...
      m_Viewport[mos][n][1].canvas()->set_scroll_anchor( Papyrus::SCROLL_ANCHOR_TOP_LEFT );
      d = Papyrus::example_group ( true, false );
      d->set_viewbox ( Papyrus::Viewbox::create ( 0.0, 0.0, 25.0, 50.0, static_cast<Papyrus::Viewbox::ALIGN> ( n ), static_cast<Papyrus::Viewbox::MEET_OR_SLICE> ( mos ) ) );
      m_Viewport[mos][n][1].canvas()->add ( d );
  ...
  for ( int i = 0; i <= 1; i++ )
  {
    m_original_viewport[i].set_size_request ( 100, 100 );
    d = Papyrus::example_group ( true, false );
    d->scale ( 0.5, 0.5 );
    m_original_viewport[i].canvas()->add ( d );
    Papyrus::Region r = d->global_extents();
    rectangle = Papyrus::Rectangle::create( r.x(), r.y(), r.width(), r.height() );
    rectangle->set_stroke ( Cairo::SolidPattern::create_rgba ( 0.0, 0.0, 1.0, 0.85 ) );
